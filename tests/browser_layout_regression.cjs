...
async function seed(request,base){
  ...
  const entryResponse=await request.post(`${base}/api/entries`,{data:{trip_id:trip.id,raw_text:'隔离种子长标题消费',recognized:{category:'meal',amount:123456.78,location:'一个非常长的测试地点名称用于窄卡宽度检查',occurred_at:localNow(),item:'一个很长很长的消费标题用于检查文本溢出和操作区布局'},client_id:'seed-entry-v36',client_revision:1}}); if(!entryResponse.ok()) throw new Error(`seed entry failed ${entryResponse.status()} ${await entryResponse.text()}`);
