...
function documentStub(){ return {body,activeElement:null,querySelector(key){ if(!nodes.has(key)) nodes.set(key,new Node(key)); return nodes.get(key); },querySelectorAll(){ return []; },createElement(tag){ return new Node(tag); },addEventListener(){}}; }
